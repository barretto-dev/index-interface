...
export async function getOutputFolders() {
  ...
  export async function deleteOutputFolder(folderName) {
    try {
      const res = await fetch(`${API_BASE}/output/folder/${folderName}`, {method: "DELETE"});
      ...
      console.log(error)
      return {status:"error", msg: "Erro inesperado na função deleteOutputFolder()"}
    }
  }